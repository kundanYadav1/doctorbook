...
const Login = () => {
  ...
  const loginUsingPassword = async (e) => {
    ...
    axios.post("https://kritaahospital.netlify.apprender.com/login", registrationData).
      then(response => {
  ...
  const loginUsingOTP = async (e) => {
    ...
    axios.post("https://kritaahospital.netlify.apprender.com/verifyOtp", registrationData)
      .then((response) => {
        if (response.data.userData.role === "admin") {
          navigate(`/admin/admindetails`);
        }
