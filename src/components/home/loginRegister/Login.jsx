...
const Login = () => {
  ...
  const loginUsingPassword = async (e) => {
    ...
    axios.post("https://doctorapp-backend-gilt.vercel.app/login", registrationData).
      then(response => {
  ...
  const loginUsingOTP = async (e) => {
    ...
    axios.post("https://doctorapp-backend-gilt.vercel.app/verifyOtp", registrationData)
      .then((response) => {
        if (response.data.userData.role === "admin") {
          navigate(`/admin/admindetails`);
        }
